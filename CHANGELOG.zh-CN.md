...
- **按需发现 Kim Service 的行业角色。** 显式绑定 Kim Service 目录后，可通过原有选路器发现自媒体、电商、求职职场、教育、副业创业五类共十五个独立角色。选择前校验每个能力合同和组件内容；角色源码保留在 Kim Service，选中角色不等于已经原生加载或通过模型交付实测。

### 修复

- **已有托管 HookPrompt 条目也会更新为一分钟超时。** 安装、更新和模板刷新统一使用 60 秒，并原位刷新旧条目；保留用户命令、其他目录的同名脚本、匹配器顺序与无关字段（issue #81）。
- **常见交付请求能够进入执行路线。** 简历、周报、客服回复等行业任务补入共享入口词表；序列化工作单保留“搜索 / 推荐流”这类普通分隔表达，同时继续遮蔽私有路径和凭据。
- **验收读取器兼容当前 Codex 桌面完成事件。** 同时识别受支持的文件变更、子代理活动和最终回复格式。Windows 尚未刷新持续写入日志的文件时间时，仍按绑定事件的时间判断新鲜度；过期事件、会话错配、失败修改和原始记录漂移继续拒绝。

## [3.1.2] - 2026-09-10

### 修复

- **npm 10 上并发生成全局投影不再失败。** 两个工作进程同时生成同一个包时，落败的一方会把自己的产物和胜出方逐字节比对，结果永远对不上，安装中止。差异来自 npm 自己的记账：npm 10 执行 `npm install <archive>` 会把归档的绝对暂存路径写成 `file:` 依赖，而这个路径带着各自的进程号和 uuid。现在在计算产物哈希前先抹掉暂存段，同一份源码在不同进程下生成的产物就一致了。那些 `file:` 地址没有任何环节会去解析——归档在安装后立即删除——所以改写不产生副作用。npm 11 记的是相对路径，从来不受影响，这也是问题只出现在旧版 npm 用户那里的原因（PR #80）。
- **缺少 lockfile 不再中断生成。** 同一段流程无条件读取三个产物元数据文件，而哪些文件存在由 npm 版本决定。现在从未写入的文件会跳过，其余读写错误照旧中止安装，不让半成品产物进入回执。
- **规划停止钩子会说清哪个条件没满足。** 之前只抛 `planning_not_verified_or_closed` 加一个为零的计数，既没指出失败条件也没给下一步。现在会报出具体未满足的完成条件，且"运行未绑定计划"的空转路径在每个生命周期事件上都有测试覆盖。
- **提示钩子的时间预算改用 Claude Code 的单位。** 钩子超时按毫秒写进了一个被 Claude Code 当秒读的字段，本想设的一分钟上限实际等于没有上限。

## [3.1.1] - 2026-09-08

### 修复

- **修复 `--roll-review` 在有效复审流程中崩溃的问题。** 命令把 `validateRuntimeCapabilityClaims` 返回的数组当成 `{ issues }` 解构，打印待更新条目后在复验阶段报错。现在测试覆盖文件读写和 CLI 入口，验证能力矩阵与证据账本中的适用绑定一起更新；仍有无法修复的问题时拒绝写盘。更新复审日期仍需维护者明确执行并提供理由，本次发布不改写已有证据日期（PR #75）。
- **滚动命令接进了 npm。** `npm run meta:runtime:evidence:roll-review`，不用再手打脚本路径。

## [3.1.0] - 2026-09-08

### 新增

- **实质不同的备选路线需要用户选择。** 决策规则识别明确备选，以及会改变范围、风险或验收的互斥路线；互补的并行工作本身不会触发提问。运行平台必须实际调用其支持的选择界面，才能把必要选择记为已回答；规则检查不代表所有平台的真实弹窗都已通过验收。
- **证据指纹有了官方刷新命令。** `npm run meta:runtime:evidence:refresh-pins` 在源码改动后重算 `repo_projection` 的源文件指纹（并提供 CI 用的 `:check` 变体），被钉住的文件被上游提交触碰后不再需要手写一次性脚本自救（issue #61）。
- **洁净房验收可以保留沙箱取证。** `--keep-temp`（或 `META_KIM_CLEAN_ROOM_KEEP_TEMP=1`）保留沙箱临时根目录；依赖安装步骤"退出 0 但输出为空且产物缺失"现在会在报告中输出显式的静默空转诊断，而不是无因失败。
- **兼容性以实测宿主能力为准。** 不因模型名称合并治理阶段、取消必要的原生选择，或宣称某个平台已经验证。兼容性研究与真实运行验收分别记录。

### 变更

- **流程图周围的信息更精简。** 任务标题只显示一处，完整说明和运行编号按需展开；顶部导航和底部不再重复标题。总览节点文字保持正文大小，放大后的完整卡片排布继续保留。
- **普通图谱重建不再自动调用模型。** 代码提取与分组在本地运行，保留已有语义内容，文档和图片不会被悄悄送去重新分析；完整性与版本对应检查继续保留。
- **启动登记不再冒充执行回报。** 仅登记启动的运行显示为尚未收到执行回写，页头和页尾使用同一公开状态。
- **Hook 上下文注入改为按内容指纹门控，不再按轮次门控。** 规划连续性在计划与上次注入逐字节一致时降级为一行指针，工作闭合后完全停止注入——重新勾开任意条目即恢复完整投影。记忆召回不再重复注入相同选集；graphify 使用提示从每次搜索命令一次降为每会话一次；子代理规则集按会话+代理去重。新增开关：`META_KIM_GRAPHIFY_CONTEXT=off`、`META_KIM_SUBAGENT_CONTEXT=off`、`META_KIM_DISABLE_RECALL_DEDUPE=1`。Codex 侧子代理治理规则的目标从所有子代理收窄为 `meta-*` 治理代理（issue #57）。
- **Live 面板如实展示运行中的工作。** 观察到的调用证据现在会把声明排队的执行者提升为可见的运行中状态（标注 `运行中·observed`），且不伪造生命周期证明；运行与会话的活动状态按同一规则推导，过期的观察证据会衰减回声明状态。声明了但从未开始的工作以虚线边框加"声明未执行"标签呈现，不再看起来像已执行。
- **流不可用时面板真的会轮询。** "Polling snapshot" 徽章背后有了真实的 10 秒快照刷新：页面隐藏时挂起、流恢复时自动拆除，不再承诺一个不存在的轮询。
- **Live 布局在测量真实卡片后重选列。** 当测量出的卡片尺寸与排布假设明显不符时，执行一次有界的校正性重排——只跑一遍，绝不循环。
- **初始状态的连线对比度增强。** 空闲、排队与结构连线使用更亮的颜色，运行中的工作保留青色强调；图例补齐观察到的运行中与声明未执行两种状态。
- **`gstack` 降级为第三方参考，不再是可安装依赖。** 默认安装不再克隆它，显式点名安装会打印警告；依赖注册表标记为 `reference_only`——这是逐步剥离非第一方依赖的第一步。

### 修复

- **平台选择不再绕过安装范围。** 只做全局安装的项目显式指定平台时，仅限定挂钩投影，不会额外生成项目里的长期代理、技能或命令（issue #76）。
- **规划挂钩不再在普通文件夹里反复报错。** 自动入口找不到可信项目时安静跳过；Claude 的显式项目目录只在 Claude 生效。显式规划命令和完整性保护继续保留。
- **任务限制不再画成待执行节点。** Live 将历史任务记录中的非目标单独保留为限制说明；已有执行证据的任务仍然显示，违反限制时标记冲突，原始记录保持完整。
- **全局检查会说明具体该修什么。** 过期挂钩列出缺失项、变更项及各运行时的正确来源；安装检查区分源码更新和安装副本损坏，缺少可信安装依据时仍明确作为诊断失败（issue #77、#78）。

- **Windows 全局同步不再倒在字节级包校验上。** 包暂存安装改用 `--no-bin-links`，解压字节与 `npm pack` 真值保持一致，不再因 bin 链接重写 CRLF shebang 而失败（issue #74）。
- **投影包单元测试不再读取用户真实包店。** `META_KIM_PROJECTION_PACKAGE_STORE_ROOT` 隔离摘要根目录，同指纹重试与并发用例在已有指纹的机器上不再失败（issue #60）。
- **Live hub 在高负载 Windows 机器上可靠启动。** 负载下返回空的 PowerShell 身份探针不再杀死守护进程启动；该失败被显式归类，不再表现为莫名退出。
- **安装器会自报家门。** 全局技能安装器直调时打印一行确认（模式、目标、技能、开关），静默的"退出 0 无输出"路径从此可以与真实空转区分（issue #59）。
- **运行时主目录被重定向时，两个安装器不再各说各话。** 依赖安装器拒绝一切根目录 junction，全局同步却接受配置好的整个运行时主目录重定向，同一个 `~/.claude/agents` 在一边合法、在另一边非法。现在两侧都记录每个配置运行时的字面根与真实根，并在预检、写入、事务重试、回滚、清理和显式依赖退役中执行同一套绑定；越出配置根的重定向、根目录改绑和只读包店写入照旧拒绝，源项目同步器也不再要求主目录里的代理符号链接。本轮以 Windows 目录 junction 实测，macOS 与 Linux 的重定向未执行（issue #62）。
- **`meta-kim release close`认识私有队列实际使用的队列 ID。** 收尾步骤原先只接受 `P-NNN`，队列改用 `M3-L05` 这类里程碑 ID 之后它一直跑不起来，文档写明的发布最后一步就这样停了。现在开关校验和 PRD `ACTIVE` 行匹配共用同一套 ID 语法；语法之外的 ID 照旧拒绝。

## [3.0.9] - 2026-09-05

### 变更

- **Live 根据依赖关系和实际显示尺寸排布工作。** 运行图按依赖深度安排位置，结合卡片与能力行的实际测量结果计算间距和可读缩放；关闭详情面板后恢复之前的画面比例。
- **Live 的历史运行更容易查找和区分。** 可配置的保留策略、统一的会话排序、过期投影补全以及暂停后可见的补收状态，帮助区分当前工作与旧记录，避免每次点击都重新扫描目录。
- **执行者状态保留证据边界。** 运行记录区分 Hook 观察、会话停止和工作完成；收到执行者事件不再被当作宿主已经执行任务的证明。

### 修复

- **Windows 安装会重试短暂的文件占用。** 事务性文件写入复用共享重试能力，覆盖改名、新建文件发布和替换文件发布；不属于 Windows 文件占用范围的错误仍会使事务失败，并保留原始失败原因。全局 Hook 替换也会重试短暂的改名失败。
- **从安装包启动的命令保留调用项目的上下文。** 稳定安装包目录不再被误当作调用方的项目目录，避免因此缩小运行时目标选择范围。
- **安装包解压兼容 Windows 盘符路径。** 从压缩包所在目录传入本地文件名，避免不同 tar 实现把盘符误认为远程主机。
- **运行时 Hook 同时保留执行者状态与规划处理。** Codex 不再让两组处理相互覆盖，Cursor 使用其原生的平铺事件格式；Codex 配置清理也尊重宿主已经删除的配置项。
- **保存的进度记录用户自己说的话，不记客户端注入的说明。** Claude Code 的 stop Hook 会先剥掉已知的客户端包装和标签前缀，再判断一段提示是否算真实目标，注入的开场白不会再变成记录下来的任务。分隔符被截断吃掉的包装会被丢弃而不是入账；Hook 不认识的方括号原样保留，避免真正以 `[` 开头的请求被误删。

### 维护

- GitHub Actions 的代码检出与 Node 环境配置依赖升级至 v7。
...
- **规划连续性已成为 Meta_Kim 自有能力。** 无需安装或调用 `planning-with-files`，Meta_Kim 也能保存按运行隔离的计划、进度、检查点、恢复证据与有界收口状态。
- **Meta_Kim Live 升级为全局本机 Hub。** 一个仅监听 loopback 的进程会列出用户明确登记的项目及其治理 Session / Run，并通过项目与运行深链进入原有图、证据和回放界面。
- **第一次治理请求会懒启动 Live。** 正常对话会收到当前运行的可点击链接；后续提示复用同一个已验证进程，不再按项目或 Session 复制服务。
- **Live 现在默认显示中文，并提供可记忆的英文切换。** 导航、运行图界面、证据、回放、空状态和无障碍标签均支持中英文；项目名、Session 标题与原始运行内容保持原语言。
- **已验证的治理项目会在首次使用时加入 Hub 目录。** 显式打开 Live 或进入治理运行后，当前带 marker 的项目会立即可选，无需扫描磁盘；用户此前明确选择跳过的项目仍保持跳过。

### 变更

- **首个依赖吸收周期已经闭环。** `planning-with-files` 只保留为历史研究证据；安装、更新、发现、provider 路由和运行时投影都不再依赖它。
- **Live 先解释运行，再展示实现细节。** 首屏现在先给出当前任务、整体进度、活跃执行者和清晰的八阶段进度条；协议标签退出主要信息层级，运行图节点只保留有助于识别工作的内容。
- **大量历史 Session 也能一眼区分。** 目录标签组合本地时间、治理阶段、安全公开标题和唯一运行短号，不再让数百条记录重复显示同一个 run-id 前缀。

### 安全

- **依赖退役现在默认失败关闭。** 安装和更新遇到无法证明由 Meta_Kim 持有的 Hook、alias、缓存或 junction 路径时会保留并阻止删除，不会误删用户内容。
- **Hub 不扫描磁盘，也不转发私人对话。** 公共目录响应只使用 opaque project ID、受限且净化的运行摘要以及显式字段白名单；仓库根目录、源码路径、原始 prompt/output 和秘密都留在服务端。

## [3.0.6] - 2026-08-26

### 变更

- **README 改为从陌生用户视角解释 Meta_Kim。** 首屏现在直接展示一句自然语言需求、执行过程中会发生什么、最终能拿到哪些交付物、收口报告长什么样，以及三步完成首次运行的路径。
- **先讲具体结果，再讲内部架构。** 示例覆盖产品实现、难定位 Bug、PR 审查和从 PRD 到代码；完整架构、平台支持与维护者参考仍然保留在可展开的技术区中。

### 视觉

- **仓库现在直接展示真实 Meta_Kim Live 体验。** 新增 1440×900 的真实控制室渲染图，用明确标注的代表性演示数据展示阶段图、worker 分支、证据抽屉、小地图和回放时间线。
- **社交卡完成刷新。** 新视觉直观对比失去协调的 AI 编码与 Meta_Kim 的意图、路由、执行、审查和学习流程。
...
- **依赖安装改为显式且可审计。** 直接运行安装器必须选择 `--all` 或明确给出 `--skills`；仅供参考的项目永不安装，已安装 provider 只有在受管 receipt 精确匹配并带有本次运行证据时才会进入 active。
- **大型产品交付运行会复用经过验证的状态。** 相同输入只有在 JSON、Markdown、reservation、SQLite 终态和递归源码闭包全部一致时才复用同一个稳定 run、数据库和 reservation；半成品或被篡改的状态会原位重建，但不删除历史。
- **Graphify 如实报告语义覆盖。** 诊断会区分“预期不进入语义图”的配置/资源文件和真正缺失的语义来源，持久化内容绑定证据，并报告真实的跨来源 ID 冲突，不再制造占位节点冒充修复。

### 修复

- **发布证据在崩溃和并发维护下更安全。** attempt journal 会汇总孤儿记录；锁同时绑定 PID 与进程启动身份；平台支持时显式 flush；远端 Release 资产下载校验改为明确 opt-in；诊断不会泄露无关绝对路径。
- **能力发现结果保持确定性。** canonical capability index 内容不变时字节和 mtime 都不变，项目 MCP 投影也不会再污染 canonical 发现结果。
- **测试不再膨胀真实 default 状态目录。** 产品 bundle 验收使用隔离状态；相同输入重复执行不会再生成另一套完整大对象。
